...
- Text domain `salient-child` (declared in `style.css`), `load_child_theme_textdomain()` from
  `salient-child/languages/`. Source strings in English; Swedish in `languages/sv_SE.po` + compiled `sv_SE.mo`
  (**filename is locale-only** — that is what load_theme_textdomain expects). No msgfmt locally: the .mo and the
  JED `.json` files for the editor scripts are generated by a small Python script (see decisions D28); the
  string table lives in that script and in the .po. Both files are committed and deployed.
- WPML String Translation notices "new .mo files" for the theme and offers a rescan; not required for the .mo
  to work. Editors can still override strings in WPML → String Translation.
...
| 3 ✅ | Index block | `fbhi/guide-index` (dynamic, ServerSideRender in editor, heading attribute), category "FBHI Guide", auto-append on root when absent. Verified in editor + frontend 2026-09-02. |
| 4 ✅ | Components | Block styles on core/group (Info box, Callout, Checklist) + core/separator (Print page break); patterns in category "FBHI Guide" restricted to `guide`. Editor canvas loads guides.css + Montserrat and mirrors the chapter accent as CSS vars. |
| 5 ✅ | Print | `guides-print.css` (media=print): hides header/footer/global sections/sidebar/nav, outlines boxes, avoids breaks in headings/boxes, honours the page-break style. Checked by injecting the sheet as screen CSS in Chrome. |
| 6 ✅ | i18n + menu | `languages/sv_SE.po/.mo` (theme .mo must be named `{locale}.mo`, not `{domain}-{locale}.mo` — load_theme_textdomain looks for `$path/$locale.mo`), JED JSON for the two editor scripts, wpml-config.xml, menu highlighting. Swedish verified on frontend. Menu entry itself still to be added by Daniel. |
| 7 ✅ | Content | All four chapters loaded on dev (docx → blocks: H2 kept, bold sub-headings → H3, "Fundera på:"-style lead + list → Callout, other lead + list → bold paragraph + list; manual TOC and title dropped). Converter lives in the session scratchpad; re-run needs the docx files in `incoming/`. Review round with Daniel pending. |
